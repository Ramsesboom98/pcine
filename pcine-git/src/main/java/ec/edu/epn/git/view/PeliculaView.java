...
public class PeliculaView {	
	...
	int desicionPelicula=3;
	int desicionFuncion=6;
	String desicionAsiento= "C3";
	String nombreCliente= "1765420864 Alvaro Mendoza";
	...
	System.out.println(desicionPelicula);
	
	imprimirFunciones(cFuncion.findAllFuncionesByIdPelicula(3));
	
	System.out.println("Ingrese el numero de la Funcion que desea ver: ");
	System.out.println(desicionFuncion);
	
	System.out.println("Asientos disponibles para la funcion.");
	imprimirAsientos(cFuncion.findFuncionById(desicionFuncion).getSala().getAsientos());
	System.out.println("Ingrese el asiento que desea: ");
	System.out.println(desicionAsiento);
	
	String pp=mapeo(desicionAsiento);
	cFuncion.cambioEstadoAsiento(Integer.parseInt(pp.substring(0,1)), Integer.parseInt(pp.substring(1,pp.length()))-1,desicionFuncion);
	imprimirAsientos(cFuncion.findFuncionById(desicionFuncion).getSala().getAsientos());
	
	System.out.println("Porfavor ingresar su numero de cedula y nombre completo separados por un  espacio.");
	System.out.println(nombreCliente);
	
	StringTokenizer token = new StringTokenizer(nombreCliente, " ");
	...
	cBoleto.addBoleto(new BoletoDTO(0, 5.00 , fechaAct.toString(), cFuncion.findFuncionById(desicionFuncion), desicionAsiento));
